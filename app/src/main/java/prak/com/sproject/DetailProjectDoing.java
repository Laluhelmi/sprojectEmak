...
/**
 * Created by Yoza on 10/06/2016.
 */

public class DetailProjectDoing extends AppCompatActivity {

    ProgressDialog prg;
    RestClient rest;
    String idproject,titleproject;
    String[] usernametim,jabatantim;
    ArrayList<BeanClassForDoing> datadoing;
    ArrayList<BeanClassForDetail> datadetail;
    DetailDoingAdapter detaildoingadapter;
    ListView listdoing;
    TextView titleproo;
    TextView judul;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.detailproject_doing);
        ...
        listdoing=(ListView)findViewById(R.id.listviewdoing);
        ...
        datadoing=new ArrayList<BeanClassForDoing>();
    ...
    private void tampildoing(String idproject) {
        ...
        StringRequest todo=new StringRequest(Request.Method.GET, "http://servicescrum.16mb.com/app/show_doing_sprint/" + idproject + "?X-API-KEY=tifuad", new Response.Listener<String>() {
            @Override
                ...
                try {
                    ...
                    if (status) {
                        ...
                        for (int i = 0; i < detailtodo.length(); i++) {
                            ...
                            String judultimvar = moredetailtodo.getString("username");
                            //String estimasitimvar = moredetailtodo.getString("estimasi");
                            // String statustodotimvar=moredetailtodo.getString("status");

                            BeanClassForDoing beandetaildoing = new BeanClassForDoing();
                            beandetaildoing.setJuduldoing(judultimvar);
                            //beandetail.setJabatantimku(jabatantimvar);
                            //menu.add(i+1+"."+beandetail.getUsernametimku()+"("+beandetail.getJabatantimku()+")");

                            datadoing.add(beandetaildoing);


                        }


                        detaildoingadapter = new DetailDoingAdapter(DetailProjectDoing.this, datadoing);
                        //listtim.setAdapter(detailadapter);
                        listdoing.setAdapter(detaildoingadapter);
...
//                        jabatantim = new String[detailpro.length()];
                        ...
                        objek = new DetailProjectAdapter(DetailProjectDoing.this, datadetail);
                        //listtim.setAdapter(detailadapter);
